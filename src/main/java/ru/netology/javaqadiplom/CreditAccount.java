...
public class CreditAccount extends Account {
    ...
    public CreditAccount(int initialBalance, int creditLimit, int rate) {
        if (rate <= 0) {
            throw new IllegalArgumentException(
                    "Ставка кредитования для расчета долга за отрицательный баланс" +
                            " не может быть отрицательной, а у вас: " + rate);
        }
        if (initialBalance < 0) { // добавил
            throw new IllegalArgumentException(
                    "Начальный баланс для счета не может быть отрицательным, а у вас: " + initialBalance
            );
        }
        if (creditLimit <= 0) { // добавил
            throw new IllegalArgumentException(
                    "Максимальная сумма которую можно задолжать банку не может" +
                            " быть отрицательной, а у вас:" + creditLimit);
        }
    ...
    public boolean pay(int amount) {
        ...
        if (balance + creditLimit >= amount) {  // при условии что  (балнас + лимита) выше суммы покупки
     ...
     * @return
     */
    // 1. Переменная balance обновляется некорректно, путем присвоения
    ...
    public boolean add(int amount) {
        ...
        balance = balance + amount; // было balance = amount, изменил
        return true;
    }
    ...
    public int yearChange() {
        if (balance < 0) {    // По условию, расчет на отрицательный баланс
